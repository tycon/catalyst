...
# We need these before we compile catalyst
FRONT_END_SOURCES :=		\
...
SPEC_PARSER_SOURCES :=		\
	specparser/spec.lex.sml	\
	specparser/spec.grm.sig	\
	specparser/spec.grm.sml
...
specparser/%.lex.sml: specparser/%.lex
	$(MAKE) -C specparser $(@F)
specparser/%.grm.sig specparser/%.grm.sml: specparser/%.grm
	$(MAKE) -C specparser $(<F).sig $(<F).sml
...
.PHONY: smlnj-catalyst
smlnj-catalyst: $(FRONT_END_SOURCES) $(SPEC_PARSER_SOURCES)
...
smlnj-mlton:
	$(MAKE) dirs 
	$(MAKE) SMLNJ_CM_SERVERS_NUM=4 smlnj-catalyst
